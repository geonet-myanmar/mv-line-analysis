...
This project performs a morphometric and routing-efficiency analysis on medium-voltage grid line geometries stored in `All_states_region_MVLine_tsp_level.json`.
...
This repository analyzes the dataset **Myanmar - Existing Grid Medium Voltage Line Data** published by **Open Development Myanmar** and mirrored on the Open Development Mekong Datahub.

- Dataset page: <https://data.thailand.opendevelopmentmekong.net/en/dataset/myanmar-existing-grid-medium-voltage-line-data>
- Dataset mirror with searchable metadata: <https://data.opendevelopmentmekong.net/dataset/myanmar-existing-grid-medium-voltage-line-data>
- Original lineage / upstream source: <https://energydata.info/dataset/myanmar-existing-grid-medium-voltage-line-data>

### Source Metadata

Based on the Open Development Mekong dataset metadata:

- Publisher: `Open Development Myanmar`
- Dataset title: `Myanmar - Existing Grid Medium Voltage Line Data`
- Topic categories: `Electricity infrastructure`, `Electricity production`
- Dataset reference date: `November 30, 2016`
- Date uploaded: `September 3, 2017, 12:34 (UTC)`
- Date modified: `May 28, 2021, 01:56 (UTC)`
- Spatial reference system: `WGS 84 (EPSG:4326) (CRS:84)`
- Format(s) listed: `GeoJSON`, `SHP`, `DB_TABLE`, `WMS`
- Version: `1.0`
- License: `Creative Commons Attribution`
- Responsible party listed in metadata: `Shaky Sherpa` (`ss3491@columbia.edu`, `energydata@worldbankgroup.org`)

### Source Description

The dataset description published by Open Development Myanmar states that the data were collected and digitized as input to geospatial least-cost planning for universal electricity access by 2030 under the World Bank-funded Myanmar National Electrification Plan (NEP). The dataset covers the distribution service area of the Electricity Supply Enterprise (ESE), Myanmar’s national utility, and the data collection was undertaken by the Sustainable Engineering Lab / Earth Institute in collaboration with Resource and Environment Myanmar (REM). Sources: [Open Development Mekong mirror](https://data.opendevelopmentmekong.net/dataset/myanmar-existing-grid-medium-voltage-line-data), [ENERGYDATA.INFO](https://energydata.info/dataset/myanmar-existing-grid-medium-voltage-line-data).
...
The repository file is a GeoJSON `FeatureCollection` containing `LineString` features. Each feature typically includes:
...
The source metadata lists the spatial reference system as `WGS 84 (EPSG:4326) (CRS:84)`. The coordinates are therefore treated as geographic coordinates (`longitude`, `latitude`). For that reason, this project does **not** use simple planar Euclidean distance on raw degrees. Instead, it computes **great-circle distances** using the haversine formula so all lengths are expressed in meters/kilometers on the earth’s surface.
...
The Open Development Mekong metadata lists the dataset license as `Creative Commons Attribution`, and the upstream ENERGYDATA entry lists `Creative Commons Attribution 4.0`. Before publishing, confirm which attribution text you want to reproduce in the repository and whether you are redistributing the original raw file, a derived file, or both. Sources: [ODM metadata](https://data.opendevelopmentmekong.net/dataset/myanmar-existing-grid-medium-voltage-line-data), [ENERGYDATA.INFO](https://energydata.info/dataset/myanmar-existing-grid-medium-voltage-line-data).

If needed, keep the script public and store the raw dataset privately.

## Attribution

If you publish this repository, include attribution to the data source. A practical attribution line is:

```text
Source data: "Myanmar - Existing Grid Medium Voltage Line Data", published by Open Development Myanmar via Open Development Mekong Datahub, with lineage to ENERGYDATA.INFO / Columbia University Earth Institute. License: Creative Commons Attribution.
```
